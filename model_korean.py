...
class GPT2PPLV2:
    def __init__(self, device="cuda", model_id="nlpai-lab/kullm-polyglot-5.8b-v2"):
        self.device = device
        ...
        self.model = AutoModelForCausalLM.from_pretrained(model_id, quantization_config=self.bnb_config, device_map={"":0})
        ...
        self.t5_model = T5ForConditionalGeneration.from_pretrained("paust/pko-t5-small", device_map={"":0})
        self.t5_tokenizer = T5TokenizerFast.from_pretrained("paust/pko-t5-small", model_max_length=512) #256
    ...
    def unmasker(self, text, num_of_masks):
        ...
        texts = [x.replace("<pad>", "").replace("</s>", "").replace("\n\n", "").replace("\n","").strip() for x in results]
...
###############################################
    ...
    def maskRandomWord(self, text, ratio):
        span = 2
        tokens = text.split(' ') # 띄어쓰기 단위로 token화
        mask_string = '<<<mask>>>'
    ...
    def call_1_1(self, sentence, chunk_value):
        ...
        labels = []
        for line in lines:
            if re.search("[ㄱ-ㅎ가-힣a-zA-Z0-9]+", line) == None: #[a-zA-Z0-9]+",
                continue
...
###############################################
    ...
    def call_1(self, sentence):
        ...
        total_valid_char = re.findall("[ㄱ-ㅎ가-힣a-zA-Z0-9]+", sentence) #[a-zA-Z0-9]+
        ...
        for i, line in enumerate(lines):
            if re.search("[ㄱ-ㅎ가-힣0-9]+", line) == None:
                continue
            if len(offset) > 0:
                line = offset + line
                offset = ""
